...
def run_command(
    ...
    try:
        ...
        if verbose:
            click.echo(
                click.style(f"Error running command: {' '.join(cmd)}", fg="red"),
                err=True,
            )
...
def dump_settings(
    ...
    if verbose:
        click.echo(
            f"Dumping {django_settings_module} with {config_file} -> {output_file}"
        )
    ...
    if verbose:
        click.echo(click.style("", fg="blue"), err=True)
        click.echo(
            click.style(
                f"DJANGO_SETTINGS_MODULE: {django_settings_module} \\", fg="blue"
            ),
            err=True,
        )
        click.echo(
            click.style(f"SERVICE_VARIANT: {env.get('SERVICE_VARIANT')} \\", fg="blue"),
            err=True,
        )
        click.echo(
            click.style(f"{config_env_var}: {config_file} \\", fg="blue"), err=True
        )
...
def process_branch(branch: str, output_dir: Path, verbose: bool = False) -> None:
    ...
    click.echo(
        click.style(f"=== Processing branch: {branch} ===", fg="blue", bold=True)
    )
    ...
    with click.progressbar(CONFIGS, label="Processing configurations") as configs:
        for django_settings_module, config_file in configs:
            output_filename = generate_output_filename(
                django_settings_module, config_file
            )
...
def main(
    main_branch: str, feature_branch: str, main_dir: str, branch_dir: str, verbose: bool
) -> None:
    """
    ...
    try:
        ...
        try:
            ...
            run_command(diff_cmd, verbose=verbose)
            ...
            if e.returncode == 1:
                ...
                click.echo(
                    click.style(
                        "Differences found between branches (shown above)", fg="yellow"
                    )
                )
            else:
                click.echo(
                    click.style(
                        f"Diff command failed with return code {e.returncode}", fg="red"
                    ),
                    err=True,
                )
                if e.stderr:
            ...
            click.echo(
                click.style(
                    "diff command not found - skipping comparison", fg="yellow"
                ),
                err=True,
            )
        except Exception as e:
        ...
        click.echo("  # Diff all settings at once:")
        ...
        if master_files:
            click.echo("  # Diff settings by file:")
            for file in master_files:
                click.echo(
                    f"  echo {file.name} | xargs -I {{}} diff -u {main_dir_path}/{{}} {branch_dir_path}/{{}}"
                )
